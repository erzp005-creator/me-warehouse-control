...
v1.19.0 -- Receiving and putaway: an admin can reverse a single PO receipt (the double-scan fix) via `record_unreceive`, which backs the qty out of the warehouse inventory pool (so it lands even after putaway moved the goods), recomputes PO status, and emits `receipt.cancelled/1` so downstream inbound counts stay in step; `GET /admin/purchase-orders/<id>/receipts` + `POST /admin/receipts/<id>/unreceive` plus receipt history and a per-row Unreceive control in the Receiving modal. The Put-Away supervisor dashboard becomes an auto-fill grid of bin tiles (red when a staging bin holds work, grey when empty) with a per-bin CSV export, and `/putaway/pending` now surfaces the priority-1 preferred bin. Preferred bins are constrained to a strict Pickable-only priority hierarchy: both write paths reject non-Pickable bins, a deferrable `UNIQUE(item_id, priority)` constraint (migration 069) plus an auto-resequence keep each item's bins a contiguous 1..K, and the rejection is surfaced on the admin page and the handheld. Mobile: PutAwayScreen shows the rejection reason; versionCode 8 -> 9, version 1.19.0, APK rebuild owed. See the [v1.19.0 release](https://github.com/hightower-systems/sentry-wms/releases/tag/v1.19.0).
